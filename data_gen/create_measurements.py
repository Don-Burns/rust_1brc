...
def build_weather_station_name_list():
    ...
    station_names = []
    with open(Path(__file__).parent / "weather_stations.csv", "r") as file:
...
def build_test_data(weather_station_names, num_rows_to_create):
    ...
    try:
        with open(Path(__file__).parent / "../data/measurements.txt", "w") as file:
            progress = 0
    ...
    file_size = os.path.getsize(Path(__file__).parent / "../data/measurements.txt")
